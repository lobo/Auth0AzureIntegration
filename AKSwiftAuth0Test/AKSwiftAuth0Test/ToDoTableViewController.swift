...
class ToDoTableViewController: UITableViewController, NSFetchedResultsControllerDelegate, ToDoItemDelegate {
    ...
    override func viewDidLoad() {
        ...
        let client = MSClient(applicationURLString: "https://akswiftauth0test.azurewebsites.net")
        
        let token = Auth0Keychain().retrieveDelegateToken()
        let userId = Auth0Keychain().retrieveProfile()?.userId
        ...
        do {
            ...
            error = error1
            Auth0Alert().showMessage("Unresolved error \(error), \(error?.userInfo)", sender: self)
            abort()
        }
    ...
    func onRefresh(sender: UIRefreshControl!) {
        ...
        self.table!.pullWithQuery(self.table?.query(), queryId: "AllRecords") {
            ...
            if error != nil {
                ...
                Auth0Alert().showMessage("Error: \(error!.description)", sender: self)
                
                // We will just discard our changes and keep the servers copy for simplicity                
                if let opErrors = error!.userInfo[MSErrorPushResultKey] as? Array<MSTableOperationError> {
                    for opError in opErrors {
                        Auth0Alert().showMessage("Attempted operation to item \(opError.itemId)", sender: self)
                        if (opError.operation == .Insert || opError.operation == .Delete) {
                            Auth0Alert().showMessage("Insert/Delete, failed discarding changes", sender: self)
                            opError.cancelOperationAndDiscardItemWithCompletion(nil)
                        } else {
                            Auth0Alert().showMessage("Update failed, reverting to server's copy", sender: self)
